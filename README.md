...
**Challenges**
The project was fairly straightforward and posed little challenge initially, and it was only during the use and implementation of Dash was it challenging. This is because of my own unfamiliarity with the module despite having read the documentation and using AI to constantly debug code and mistakes. In addition the use of 2 varying inputs also proved to be laborious as the result you see is not what was desired. The vision of this project would be one that would allow the user to have great freedom and autonomy in the choice of inputs for every graph and not just one drop down menu for every graph that would change the variables for every single graph at once. I wanted to avoid this as it would prove a hindrance to data analysis as the user would not be able to compare the sales or profits of different categories and sub-categories side to side, which put more of the burden on the user to recall the information. This would be counter of the purpose and objectives of this project but due to time constraints and my own expertise for simplicity I opted for this drop-down method, given the other would require a significant number of lines be written in python which exceeded my own deadline.
In addition you may face optimization issues due to how this project was run on an old windows 7 laptop 32 bit with outdated software (Jupyterlab 2.1.5) and modules from python (3.8). If you encounter any issues with the code and cannot explain them please look into these specific areas as they may be the cause even through it should cause no issues.   
